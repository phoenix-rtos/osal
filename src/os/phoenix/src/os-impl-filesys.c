...
/****************************************************************************************
                                   GLOBAL DATA
 ***************************************************************************************/
const char OS_PHOENIX_DEVICEFILE_PREFIX[] = "/dev/";
...
/* --------------------------------------------------------------------------------------
    Name: OS_Phoenix_FileSysAPI_Impl_Init
...
int32 OS_FileSysStartVolume_Impl(const OS_object_token_t *token)
{
    ...
    if (local->fstype == OS_FILESYS_TYPE_UNKNOWN &&
        strncmp(local->device_name, OS_PHOENIX_DEVICEFILE_PREFIX, sizeof(OS_PHOENIX_DEVICEFILE_PREFIX) - 1) == 0)
    {
        /*
