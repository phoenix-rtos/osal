...
#include "generic_phoenix_bsp_internal.h"

OS_BSP_GenericPhoenixGlobalData_t OS_BSP_GenericPhoenixGlobal;
...
void OS_BSP_Initialize(void)
{
    ...
    status = mutexCreate(&OS_BSP_GenericPhoenixGlobal.AccessMutex);
    if (status < 0)
    {
        BSP_DEBUG("mutexCreate: %s\n", strerror(status));
    }
}
...
void OS_BSP_Lock_Impl(void)
{
    ...
    status = mutexLock(OS_BSP_GenericPhoenixGlobal.AccessMutex);
    if (status < 0)
    {
        BSP_DEBUG("mutexLock: %s\n", strerror(status));
    }
}
...
void OS_BSP_Unlock_Impl(void)
{
    ...
    status = mutexUnlock(OS_BSP_GenericPhoenixGlobal.AccessMutex);
    if (status < 0)
    {
        BSP_DEBUG("mutexUnlock: %s\n", strerror(status));
    }
}
...
int main(int argc, char *argv[])
{
    ...
    memset(&OS_BSP_GenericPhoenixGlobal, 0, sizeof(OS_BSP_GenericPhoenixGlobal));
